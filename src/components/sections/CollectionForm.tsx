...
import { submitCollectionRequest } from '@/lib/api';
...
export default function CollectionForm() {
  ...
  const handleSubmit = async () => {
    if (submitting || !orgType || !dataDestruction || !assetReporting || !recurring) return;
    setSubmitting(true);
    setSubmitError('');
    try {
      await submitCollectionRequest({
        orgType, contactName, company, position, email, phone,
        address, suburb, postcode, buildingLevel, loadingAccess,
        items: items.map((item) => ({ equipment: item.equipment, quantity: item.quantity })),
        dataDestruction, assetReporting, recurring, preferredDate,
        accessInstructions, additionalInfo,
      });
      setSubmitted(true);
    } catch (error) {
      ...
      <section id="request-collection" className="bg-cream-100 section-py">
        <div className="container-px">
          <div className="max-w-2xl mx-auto text-center reveal">
            ...
            <p className="text-lg text-ink-600 leading-relaxed mb-2 text-pretty">Thank you for contacting Sydney TechCycle.</p>
            <p className="text-base text-ink-500 leading-relaxed mb-8 text-pretty">Our team will review your collection requirements and contact you regarding the next step.</p>
            <div className="p-5 rounded-xl bg-cream-50 border border-ink-100 mb-8"><p className="text-sm text-ink-500">Please note: This is a request, not a confirmed booking. Our team will be in touch to finalise details.</p></div>
            <button onClick={() => {
              setSubmitted(false); setStep(0); setOrgType(''); setItems([]);
              setDataDestruction(''); setAssetReporting(''); setRecurring(''); setSubmitError('');
            }} className="btn-secondary">Submit Another Request</button>
          </div>
        </div>
      </section>
    );
  ...
  return (
    <section id="request-collection" className="bg-cream-100 section-py">
      <div className="container-px"><div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-10 reveal reveal-delay-1">
          {STEPS.map((label, i) => {
            const Icon = STEP_ICONS[i]; const isComplete = i < step; const isCurrent = i === step;
            return <div key={label} className="flex items-center flex-1 last:flex-none">
              <div className="flex flex-col items-center gap-2">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 ${isComplete ? 'bg-forest-600 text-cream-50' : isCurrent ? 'bg-ink-900 text-cream-50 ring-4 ring-ink-900/10' : 'bg-cream-200 text-ink-400 border border-ink-200'}`}>
              ...
              {i < STEPS.length - 1 && <div className={`h-0.5 flex-1 mx-2 rounded transition-colors duration-300 ${isComplete ? 'bg-forest-600' : 'bg-ink-200'}`} />}
            </div>;
          })}
        </div>
        ...
        <div className="bg-cream-50 border border-ink-100 rounded-2xl p-6 lg:p-10 reveal reveal-delay-2">
          {step === 0 && <div className="animate-fade-up">
            <h3 className="font-display font-semibold text-xl text-ink-900 mb-2">Organisation Type</h3><p className="text-sm text-ink-500 mb-6">What type of organisation are you?</p>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">{ORG_TYPES.map((type) => <button key={type} onClick={() => setOrgType(type)} className={`p-4 rounded-xl border text-sm font-medium text-left transition-all duration-300 ${orgType === type ? 'border-forest-600 bg-forest-600/8 text-forest-800 ring-2 ring-forest-500/10' : 'border-ink-200 bg-cream-50 text-ink-600 hover:border-forest-300'}`}>{type}</button>)}</div>
            <div className="mt-6 p-4 rounded-xl bg-ink-100 border border-ink-200"><p className="text-xs text-ink-500">Sydney TechCycle is a B2B service. We do not service households or residential customers.</p></div>
          </div>}

          {step === 1 && <div className="animate-fade-up space-y-4"><h3 className="font-display font-semibold text-xl text-ink-900 mb-2">Contact Details</h3><p className="text-sm text-ink-500 mb-6">Who should we contact about this collection?</p><div className="grid sm:grid-cols-2 gap-4">
          ...
          {step === 2 && <div className="animate-fade-up space-y-4"><h3 className="font-display font-semibold text-xl text-ink-900 mb-2">Collection Location</h3><p className="text-sm text-ink-500 mb-6">Where should we collect the equipment?</p><div className="grid sm:grid-cols-2 gap-4">
          ...
          {step === 3 && <div className="animate-fade-up"><h3 className="font-display font-semibold text-xl text-ink-900 mb-2">Equipment Details</h3><p className="text-sm text-ink-500 mb-6">What equipment needs collecting? Add approximate quantities.</p>
            {items.length === 0 && <div className="text-center py-8 border-2 border-dashed border-ink-200 rounded-xl"><Monitor className="w-8 h-8 text-ink-300 mx-auto mb-2" /><p className="text-sm text-ink-400">No equipment added yet</p></div>}
            <div className="space-y-3">{items.map((item, i) => <div key={i} className="flex gap-3 items-start"><div className="flex-1"><select value={item.equipment} onChange={(e) => updateItem(i, 'equipment', e.target.value)} className="input-premium appearance-none cursor-pointer">{EQUIPMENT_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}</select></div><div className="w-32"><input value={item.quantity} onChange={(e) => updateItem(i, 'quantity', e.target.value)} className="input-premium" placeholder="Qty" /></div><button onClick={() => removeItem(i)} className="w-11 h-11 rounded-xl bg-ink-100 text-ink-500 flex items-center justify-center shrink-0 transition-colors hover:bg-red-50 hover:text-red-600"><X className="w-4 h-4" /></button></div>)}</div>
          ...
          {step === 4 && <div className="animate-fade-up space-y-5"><h3 className="font-display font-semibold text-xl text-ink-900 mb-2">Requirements</h3><p className="text-sm text-ink-500 mb-6">Any specific requirements for this collection?</p>
            <div><label className="text-sm font-medium text-ink-700 mb-2 block">Data destruction required?</label><div className="flex gap-3">{(['YES','NO','NOT SURE'] as const).map((opt) => <button key={opt} onClick={() => setDataDestruction(opt)} className={`px-5 py-2.5 rounded-xl border text-sm font-medium transition-all duration-300 ${dataDestruction === opt ? 'border-forest-600 bg-forest-600/8 text-forest-800' : 'border-ink-200 bg-cream-50 text-ink-600 hover:border-forest-300'}`}>{opt === 'NOT SURE' ? 'Not Sure' : opt}</button>)}</div></div>
            <div><label className="text-sm font-medium text-ink-700 mb-2 block">Asset reporting required?</label><div className="flex gap-3">{(['YES','NO'] as const).map((opt) => <button key={opt} onClick={() => setAssetReporting(opt)} className={`px-5 py-2.5 rounded-xl border text-sm font-medium transition-all duration-300 ${assetReporting === opt ? 'border-forest-600 bg-forest-600/8 text-forest-800' : 'border-ink-200 bg-cream-50 text-ink-600 hover:border-forest-300'}`}>{opt}</button>)}</div></div>
            <div><label className="text-sm font-medium text-ink-700 mb-2 block">Recurring collections?</label><div className="flex gap-3">{(['YES','NO'] as const).map((opt) => <button key={opt} onClick={() => setRecurring(opt)} className={`px-5 py-2.5 rounded-xl border text-sm font-medium transition-all duration-300 ${recurring === opt ? 'border-forest-600 bg-forest-600/8 text-forest-800' : 'border-ink-200 bg-cream-50 text-ink-600 hover:border-forest-300'}`}>{opt}</button>)}</div></div>
          ...
          {step === 5 && <div className="animate-fade-up"><h3 className="font-display font-semibold text-xl text-ink-900 mb-2">Review Your Request</h3><p className="text-sm text-ink-500 mb-6">Please review the details before submitting.</p><div className="space-y-4">
          ...
          {submitError && <div role="alert" className="mt-6 p-4 rounded-xl border border-red-200 bg-red-50 text-sm text-red-700">{submitError}</div>}
          <div className="flex items-center justify-between mt-8 pt-6 border-t border-ink-100">
            {step > 0 ? <button onClick={() => setStep(step - 1)} disabled={submitting} className="inline-flex items-center gap-2 px-5 py-3 text-sm font-medium text-ink-600 transition-colors hover:text-ink-900 disabled:opacity-50"><ArrowLeft className="w-4 h-4" />Back</button> : <span />}
            {step < STEPS.length - 1 ? <button onClick={() => canProceed() && setStep(step + 1)} disabled={!canProceed()} className="inline-flex items-center gap-2 px-6 py-3 bg-yellow-400 text-ink-950 font-semibold text-sm rounded-full transition-all duration-300 ease-out-expo hover:bg-forest-600 hover:text-cream-50 hover:-translate-y-0.5 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:translate-y-0">Continue<ArrowRight className="w-4 h-4" /></button> : <button onClick={handleSubmit} disabled={submitting} className="inline-flex items-center gap-2 px-6 py-3 bg-yellow-400 text-ink-950 font-semibold text-sm rounded-full transition-all duration-300 ease-out-expo hover:bg-forest-600 hover:text-cream-50 hover:-translate-y-0.5 disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:translate-y-0">{submitting ? 'Submitting…' : 'Submit Collection Request'}<Send className="w-4 h-4" /></button>}
          </div>
        </div>
      </div></div>
    </section>
